...
function mudtar_wcms_concatenate_length_on_purchase( $plan, $args ) {
    ...
    if ( $item_quantity > 1 ) {
        $user_membership =
            new WC_Memberships_User_Membership( $args['user_membership_id'] );

        // Get the current end date from the membership.
        $end_date = strtotime( $user_membership->get_end_date() );
        // Subtract the access length that has just been added to the
        // user membership after the purchase.
        $end_date = strtotime( '- ' . $plan->get_access_length(), $end_date );
        // Calculate the access length amount to add by multiplying the
        // item quantity by the membership plan's access length amount.
        $access_length_amount_to_add =
            $plan->get_access_length_amount() * $item_quantity;
...
add_action(
    ...
    'mudtar_wcms_concatenate_length_on_purchase',
    10,
    2
);
